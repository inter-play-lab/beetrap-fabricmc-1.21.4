...
public class PhysicalAgent extends Agent {
    ...
    public PhysicalAgent(ServerWorld world) {
        this.world = world;
        ...
        this.beeEntity.setPos(0, 1, 0);
        this.beeEntity.setCustomNameVisible(true);
        this.beeEntitySay("Bip Buzzley");
    ...
    public void beeEntitySay(String message) {
        this.beeEntity.setCustomName(Text.of(message));
    }
    ...
    public void tick() {
        ...
        if(this.ticks >= 20) {
            if(!this.messageQueue.isEmpty()) {
                this.beeEntitySay(this.messageQueue.removeFirst());
            }
        ...
        this.getGpt4oLatestResponseAsync(message).whenComplete(
                (response, throwable) -> {
                    ...
                    for(int i = 0; i < t.length; ++i) {
                        if(i != 0 && i % 10 == 0) {
                            PhysicalAgent.this.beeEntitySay(sb.toString());
