...
public class SimpleVoiceChatPlugin implements VoicechatPlugin {
    ...
    private void onStartWorldTick(ClientWorld clientWorld) {
        ...
        if (isRecording && currentTime - lastAudioTimestamp > VOICE_TIMEOUT_MS && audioBuffer.size() > 0) {
            LOG.info("Voice transmission timeout detected! Time since last audio: {} ms, Buffer size: {} bytes", 
